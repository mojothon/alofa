...
| 过载 + 长 prompt 的抢占活锁 | `missing` | 探针（未固化为门）：容量 10 块、4 条请求各 20 prompt（16 行一拍 → 跨两片）+ 3 生成、块 4 字节 → 300 拍、**190 次抢占、0 个 token 产出**；`max_wait_ticks=8`（默认）时四条全部 `n_out=0`，取 900 时两条完成、两条 0 输出。根因：抢占丢弃已喂的部分（`done` 归零），而 prompt 需两片才喂完，于是请求总在第一片之后被抢占、永远喂不完——等待上限越小，强制 prefill 越频繁，抢占越多，反而更糟。**不是记账问题**：两本账最终一致（3/3）。修法要动「抢占是否保留部分 prefill 的进度」这条契约 → 设计级，未修；`kv_room.mojo` 里「half-delivered prompt」的提示正是同一处 |
