...
| KV 物理块池（含 `freed_blocks` 这类外部释放） | `missing` | **仍未接入调度器**：2.1 已让“不属于任何请求的块”成为可能（前缀缓存条目在请求释放后仍被树持有），所以这个字段不再是只能填 0 —— 但 `SchedInput` 侧尚未接线，**等 2.5 批张量池与 executor 一起做**，避免为接而接 |
