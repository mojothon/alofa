...
**这个门判的是"通路在工作"，不是"质量达标"。** 实测一致率 **0.8691**（输出投影
留在 fp32 的情况下；它是 2026-09-22 修掉量化分支那个行偏移多乘的 4 之后测出来
的，**修之前是 0.8164** —— 那 5 个百分点被误当成"量化质量"，其实是 bug）。项目路线
图里"一致率 ≥ 0.90"那条是**质量门，本轮没过**，账本里按实测数字记着，没有为了
让它绿而把 0.90 改小。这里 0.75 的下限只用来回答一个问题：量化通路是不是真的在
算东西 —— 解码高低半字节写反会得到 0.0，前向悄悄退回 fp32 会得到 1.0，两者都会
被这条断言抓住。下面还有一道 **0.85 的回归下限**，专门挡"不该掉的掉了"。
...
# **第二道门：不该掉的掉了。** 2026-09-22 之前实测是 **0.8164**，而那不是量化
# 质量问题 —— `project` 的量化分支里行偏移写成了 `r * out * 4`（类型化指针按
# **元素**偏移，那个 `* 4` 是 RawPtr 按字节的习惯误抄过来），于是批 > 1（整段
# prompt 一起喂）时第 1..t_rows-1 行**一个都没被写**、并且写到了 `dst` 外面。
# 去掉那个 `* 4` 之后，同一棵树、同一份 fixture 实测 **0.8691**（A/B 过）。
# 0.75 那道门**看不见**这个差别（0.8164 也在它绿区里），所以这里单独补一道：
# 退回那个 bug 会立刻红在下限 0.85 上。
comptime Q4_REGRESSION_FLOOR = Float64(0.85)
...
def test_quantised_greedy_stays_with_the_reference() raises:
    ...
    assert_true(
        rate >= Q4_REGRESSION_FLOOR,
        "量化贪心一致率掉到 "
        + String(rate)
        + "（回归下限 "
        + String(Q4_REGRESSION_FLOOR)
        + "）—— 2026-09-22 那个行偏移多乘的 4 就是把这个数压到 0.8164 的，"
        + "症状第一眼看着像\"量化质量差\"，其实是第 1 行之后的 token 一行都没被写",
    )
